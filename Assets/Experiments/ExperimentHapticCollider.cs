...
public class ExperimentHapticCollider : MonoBehaviour
{
    private ExperimentHapticPlugin HPlugin = null;
    ...
    void Start()
    {
        if (HPlugin == null)
        {
            ...
            ExperimentHapticPlugin[] HPs = (ExperimentHapticPlugin[])Object.FindObjectsOfType(typeof(ExperimentHapticPlugin));
            foreach (ExperimentHapticPlugin HP in HPs)
            {
    ...
    {
        if (collision.collider.GetComponent<ExperimentHapticCollider>() == null)
        {
            if (HPlugin != null)
            {
